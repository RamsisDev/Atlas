...
- [ ] Spawn primitives: `spawn()`, `spawn_batch()`, `spawn_sequential()` — Claude Code and API implementations
- [ ] D00 Pipeline Orchestrator + Gate Validator agent
- [ ] `delta-spec.md` standard format (ADDED / MODIFIED / REMOVED)
- [ ] Skills architecture: folder structure + `.skill.md` format
- [ ] Universal Ralph Protocol: `{agent}-state.md` template and 8-step contract

### Phase 2 — Make it safe
*Recovery, coordination, no race conditions.*

- [ ] File-scope overlap check at Gate C (replaces the entire `locks/` directory)
- [ ] Department checkpoint protocol (`dept-checkpoint.md` + D00 recovery strategies)
- [ ] Interface Contract protocol (`{Entity}.contract.md` in `contracts/`)
- [ ] Software Architect Agent (D04, read-only, max 2 iterations)
- [ ] Integration Agent (D04, contract verification + integration testing, max 3 iterations)

### Phase 3 — Make it performant
*Parallel QA, multi-feature isolation, better triage.*

- [ ] Parallel QA groups (QA Root: 6 iters · QA sub-agents: 2 iters each)
- [ ] Overview Agent read-only code access (+Read, Glob, Grep tools)
- [ ] Multi-feature isolation via git branches + `shared-proposals/`
- [ ] Escalation Router agent + Dependency Vulnerability Scanner (D05)
- [ ] Constitution Steward (batches all `shared-proposals/` in a single controlled pass)
